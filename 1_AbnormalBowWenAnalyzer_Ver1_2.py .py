    ...
    if ErrCode==0:
        ...
        for Push in Post.getPushList():
            if Push.getType() == PTT.PushType.Push:
                PushCount+=1
                if PushCount<=CalUnderPush:
                    tmpIDList.append(Push.getAuthor())
...
def main():
    ...
    if SearchID==False:
        ...
        print('The rate is:',PostOverPushBoundary/totalPost)
        print(TargetID,' post gap under boundary number is:',PostGapUnderBoundary)
        print('The rate is:',PostGapUnderBoundary/totalPost)
    ...
    #Same ID
    global IDList
    IDList=np.array(IDList).flatten()
    print(IDList.shape,'shape')
    C=Counter(IDList).most_common()
    print(C,'Counter')
    #Logout
    PTTBot.logout()
